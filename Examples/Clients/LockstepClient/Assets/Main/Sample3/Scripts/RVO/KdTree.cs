...
namespace RVO
{
    ...
    internal class KdTree
    {
        ...
        internal int queryNearAgent(Vector2 point, FP radius)
        {
            FP rangeSq = FP.MaxValue;
            int agentNo = -1;
        ...
        private void buildAgentTreeRecursive(int begin, int end, int node)
        {
            ...
            {
                agentTree_[node].maxX_ = TSMath.Max(agentTree_[node].maxX_, agents_[i].position_.x_);
                agentTree_[node].minX_ = TSMath.Min(agentTree_[node].minX_, agents_[i].position_.x_);
                agentTree_[node].maxY_ = TSMath.Max(agentTree_[node].maxY_, agents_[i].position_.y_);
                agentTree_[node].minY_ = TSMath.Min(agentTree_[node].minY_, agents_[i].position_.y_);
            }
        ...
        private void queryAgentTreeRecursive(Vector2 position, ref FP rangeSq, ref int agentNo, int node)
        {
            if (agentTree_[node].end_ - agentTree_[node].begin_ <= MAX_LEAF_SIZE)
            {
                for (int i = agentTree_[node].begin_; i < agentTree_[node].end_; ++i)
                {
                    FP distSq = RVOMath.absSq(position - agents_[i].position_);
                    if (distSq < rangeSq)
                    {
                        rangeSq = distSq;
        ...
        private void queryAgentTreeRecursive(Agent agent, ref FP rangeSq, int node)
        {
            ...
            {
                FP distSqLeft = RVOMath.sqr(TSMath.Max(0.0f, agentTree_[agentTree_[node].left_].minX_ - agent.position_.x_)) + RVOMath.sqr(TSMath.Max(0.0f, agent.position_.x_ - agentTree_[agentTree_[node].left_].maxX_)) + RVOMath.sqr(TSMath.Max(0.0f, agentTree_[agentTree_[node].left_].minY_ - agent.position_.y_)) + RVOMath.sqr(TSMath.Max(0.0f, agent.position_.y_ - agentTree_[agentTree_[node].left_].maxY_));
                FP distSqRight = RVOMath.sqr(TSMath.Max(0.0f, agentTree_[agentTree_[node].right_].minX_ - agent.position_.x_)) + RVOMath.sqr(TSMath.Max(0.0f, agent.position_.x_ - agentTree_[agentTree_[node].right_].maxX_)) + RVOMath.sqr(TSMath.Max(0.0f, agentTree_[agentTree_[node].right_].minY_ - agent.position_.y_)) + RVOMath.sqr(TSMath.Max(0.0f, agent.position_.y_ - agentTree_[agentTree_[node].right_].maxY_));
        ...
        private bool queryVisibilityRecursive(Vector2 q1, Vector2 q2, FP radius, ObstacleTreeNode node)
        {
            ...
            FP invLengthI = new FP(1) / RVOMath.absSq(obstacle2.point_ - obstacle1.point_);
