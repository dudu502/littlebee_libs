...
namespace RVO
{
    ...
    public class Simulator
    {
         ...
         * this agent.</param>
         */
        public int addAgent(Vector2 position, FP neighborDist, int maxNeighbors, FP timeHorizon, FP timeHorizonObst, FP radius, FP maxSpeed, Vector2 velocity)
        {
            Agent agent = new Agent();
            agent.id_ = s_totalID;
            s_totalID++;
        ...
        public int queryNearAgent(Vector2 point, FP radius)
        {
            if (getNumAgents() == 0)
                return -1;
